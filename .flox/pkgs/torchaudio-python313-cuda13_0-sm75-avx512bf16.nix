# TorchAudio with PyTorch 2.10.0 for NVIDIA Turing (SM75: T4, RTX 2080 Ti) + AVX-512 BF16
# Package name: torchaudio-python313-cuda13_0-sm75-avx512bf16
...
let
  ...
  customPytorch = (nixpkgs_pinned.python3Packages.torch.override {
    ...
    pname = "pytorch210-for-torchaudio-sm75-avx512bf16";
    patches = [];
    ninjaFlags = [ "-j32" ];
...
in
  (nixpkgs_pinned.python3Packages.torchaudio.override { torch = customPytorch; }).overrideAttrs (oldAttrs: {
    pname = "torchaudio-python313-cuda13_0-sm75-avx512bf16";
    ninjaFlags = [ "-j32" ];
    requiredSystemFeatures = [ "big-parallel" ];
    preConfigure = (oldAttrs.preConfigure or "") + ''
      export CXXFLAGS="${nixpkgs_pinned.lib.concatStringsSep " " cpuFlags} $CXXFLAGS"
    ...
    '';
    meta = oldAttrs.meta // {
      description = "TorchAudio for NVIDIA T4/RTX 2080 Ti (SM75, Turing) + AVX-512 BF16 with PyTorch 2.10.0";
