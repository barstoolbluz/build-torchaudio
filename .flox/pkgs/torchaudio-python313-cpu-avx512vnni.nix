# TorchAudio with PyTorch 2.10.0 for CPU-only + AVX-512 VNNI
# Package name: torchaudio-python313-cpu-avx512vnni
#
# CPU-only build optimized for x86-64 systems with AVX-512 VNNI instruction set.
...
let
  ...
  customPytorch = (nixpkgs_pinned.python3Packages.torch.override {
    ...
    pname = "pytorch210-for-torchaudio-cpu-avx512vnni";
    patches = [];
    ninjaFlags = [ "-j32" ];
...
in
  (nixpkgs_pinned.python3Packages.torchaudio.override {
    ...
    pname = "torchaudio-python313-cpu-avx512vnni";
    ninjaFlags = [ "-j32" ];
    requiredSystemFeatures = [ "big-parallel" ];
    preConfigure = (oldAttrs.preConfigure or "") + ''
      export CXXFLAGS="${nixpkgs_pinned.lib.concatStringsSep " " cpuFlags} $CXXFLAGS"
      export CFLAGS="${nixpkgs_pinned.lib.concatStringsSep " " cpuFlags} $CFLAGS"
      export MAX_JOBS=32
      echo "CPU-only build | CPU: AVX-512 VNNI | PyTorch: 2.10.0"
    '';
    meta = oldAttrs.meta // {
      description = "TorchAudio CPU-only + AVX-512 VNNI with PyTorch 2.10.0";
      longDescription = ''
