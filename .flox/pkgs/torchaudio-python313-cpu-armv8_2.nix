# TorchAudio with PyTorch 2.10.0 for CPU-only + ARMv8.2
# Package name: torchaudio-python313-cpu-armv8_2
#
# CPU-only build optimized for ARM systems with ARMv8.2 instruction set.
...
let
  ...
  customPytorch = (nixpkgs_pinned.python3Packages.torch.override {
    ...
    pname = "pytorch210-for-torchaudio-cpu-armv8_2";
    patches = [];
    ninjaFlags = [ "-j32" ];
...
in
  (nixpkgs_pinned.python3Packages.torchaudio.override {
    ...
    '';
    meta = oldAttrs.meta // {
      description = "TorchAudio CPU-only + ARMv8.2 with PyTorch 2.10.0";
      longDescription = ''
        CPU-only TorchAudio build with targeted optimizations:
        - CPU: ARM with ARMv8.2 instruction set (FP16, DotProd)
        - PyTorch: 2.10.0
        - Python: 3.13
        - No CUDA/GPU support
