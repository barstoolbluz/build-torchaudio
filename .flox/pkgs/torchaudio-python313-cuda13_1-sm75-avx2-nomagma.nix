# TorchAudio with PyTorch 2.10.0 for NVIDIA Turing (SM75: T4, RTX 2080 Ti) + AVX2 (no MAGMA)
# Package name: torchaudio-python313-cuda13_1-sm75-avx2-nomagma
...
let
  ...
  customPytorch = (nixpkgs_pinned.python3Packages.torch.override {
    ...
    pname = "pytorch210-for-torchaudio-sm75-avx2-nomagma";
...
in
  (nixpkgs_pinned.python3Packages.torchaudio.override { torch = customPytorch; }).overrideAttrs (oldAttrs: {
    pname = "torchaudio-python313-cuda13_1-sm75-avx2-nomagma";
    ...
    preConfigure = (oldAttrs.preConfigure or "") + ''
      ...
      export CFLAGS="${nixpkgs_pinned.lib.concatStringsSep " " cpuFlags} $CFLAGS"
      export MAX_JOBS=32
      echo "GPU Target: ${gpuArchSM} (Turing: T4, RTX 2080 Ti) | CPU: AVX2 | CUDA: 13.1"
    '';
    meta = oldAttrs.meta // {
      description = "TorchAudio for NVIDIA T4/RTX 2080 Ti (SM75, Turing) + AVX2 with PyTorch 2.10.0 (no MAGMA)";
